...
void Game::Update()
{
	...
	//						player.move_x += player.move_y;
			...
			if (player.pos_z - 15 <= goal_line_z && player.pos_x >= goal_line_x - 15 && player.pos_x <= goal_line_x + 15) {
				goal = true;
				init = true;
		...
		case SECOND_STAGE:
			...
			//	川に入った瞬間のせせらぎ音の処理
			if (player.pos_z <= -115 && player.pos_z >= -116) {
				PlaySoundFile("data/Game/SE/河原.mp3", DX_PLAYTYPE_LOOP);
			}
			if (player.pos_z <= -114 && player.pos_z >= -115) {
				StopSoundFile();
			}
			...
			if (player.pos_z <= -320 && player.pos_z >= -440) {
				...
				if (ufo.ufo_standhing) {
					...
					if (player.pos_x <= ufo.pos_x + 3 && player.pos_x >= ufo.pos_x - 3 ||
						player.pos_x <= ufo.pos_x + 3 && player.pos_x >= ufo.pos_x - 3) {
						//                car_die = true;
						/* no_die = false;
							  ufo.ufo_die = true;*/
							  //car_die = true;
